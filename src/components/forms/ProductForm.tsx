...
const ProductForm: React.FC<Props> = (props) => {
  ...
  useEffect(() => {
    if (selectedProduct && mode === 'edit') {
      const { info, specification, title, description, colors, sizes, images, price, inStock, discount, optionsType } =
        selectedProduct
      reset({
        info,
        specification,
    ...
    <section className="p-3 md:px-3 xl:px-8 2xl:px-10">
      ...
      >
        <Tab.Group>
          ...
          <Tab.Panels>
            <Tab.Panel>
              <TextField label="عنوان" name="title" control={control} errors={formErrors.title} />

              <TextArea name="description" control={control} label="معرفی" errors={formErrors.description} />
            </Tab.Panel>

            <Tab.Panel>
              <ProductImageList errors={formErrors.images} control={control} />
            </Tab.Panel>

            <Tab.Panel>
              <div className="space-y-4 md:flex md:items-baseline md:justify-evenly md:gap-x-2">
                <TextField
                  label="قیمت"
                  name="price"
                  control={control}
            ...
            <Tab.Panel>
              {mode === 'create' && isDetailsSkip ? (
                ...
                <>
                  <ProductColorForm control={control} register={register} />
                  {/*  @ts-ignore  */}
                  <DisplayError errors={formErrors.colors} />
                </>
              ) : watch('optionsType') === 'sizes' ? (
                <>
                  <ProductSizeForm control={control} register={register} />
                  {/*  @ts-ignore  */}
                  ...
                  <TableContainer tHeads={['نام', 'مقدار']}>
                    {watch('info').map((item, index) => (
                      <tr key={index} className="border-b-2 border-gray-100">
                        ...
                        <td className="p-2">
                          <textarea rows={3} className="text-field__input" {...register(`info.${index}.value`)} />
                        </td>
                      </tr>
                    ))}
                  </TableContainer>
                    ...
                    {watch('specification').map((item, index) => (
                      <tr key={index} className="border-b-2 border-gray-100">
                        <td className="my-0.5 text-right">
                          ...
                          />
                          <DisplayError errors={formErrors.specification?.[index]?.title} />
                        </td>
                        <td className="p-2">
                          <textarea
                            rows={3}
