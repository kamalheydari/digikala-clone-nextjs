...
const CategorySelector: React.FC<Props> = (props) => {
  ...
  // ? States
  const [levelOneCategories, setLevelOneCategories] = useState<ICategory[]>([] as ICategory[])

  const [levelTwoCategories, setLevelTwoCategories] = useState<ICategory[]>([] as ICategory[])

  const [levelThreeCategories, setlevelThreeCategories] = useState<ICategory[]>([] as ICategory[])

  // ? Re-Renders
  useEffect(() => {
    if (categories && selectedCategories.levelOne?._id)
      setLevelTwoCategories(categories?.filter((cat) => cat.parent === selectedCategories.levelOne?._id))

    if (categories && selectedCategories.levelTwo?._id)
      setlevelThreeCategories(categories.filter((cat) => cat.parent === selectedCategories.levelTwo?._id))
