              ...
              <div className="space-y-8 text-white">
                <div className="flex gap-x-3 text-gray-600">
                  <p className="flex items-center gap-x-1 text-sm">
                    <span className="inline-block h-6 w-6 rounded-md bg-red-500" />
                    دسته‌بندی سطح اول
                  </p>
                  <p className="flex items-center gap-x-1 text-sm">
                ...
                <ul className="space-y-8">
                  {categoriesList &&
                    categoriesList.children?.map((mainCategory) => (
                      <li key={mainCategory._id} className="rounded-md border border-gray-100 p-2 shadow">
                        <div className="rounded bg-red-500 p-2 text-center">{mainCategory.name}</div>
                          ...
                          {mainCategory.children &&
                            ...
                            mainCategory.children.map((parentCategory) => (
                              <li key={parentCategory._id} className="flex-1">
                                <div className="mt-2 rounded bg-green-500 p-2 text-center">{parentCategory.name}</div>
                                <ul className="flex flex-wrap gap-x-4">
                                  {parentCategory.children &&
                                    parentCategory.children.length > 0 &&
                                    parentCategory.children.map((childCategory) => (
                                      <li key={childCategory._id} className="flex-1">
                                        <div className="mt-2 flex-1 rounded bg-blue-500 p-2 text-center">
                                          {childCategory.name}
                                        </div>
                                      </li>
                                    ))}
