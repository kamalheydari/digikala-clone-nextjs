...
const getProduct = async (req: NextApiRequestWithUser, res: NextApiResponse) => {
  try {
    const { id } = req.query
...
const updateProduct = async (req: NextApiRequestWithUser, res: NextApiResponse) => {
  try {
...
const deleteProduct = async (req: NextApiRequestWithUser, res: NextApiResponse) => {
  try {
    if (req.user.role !== roles.ROOT) return sendError(res, 403, 'شما اجازه انجام این عملیات را ندارید')
