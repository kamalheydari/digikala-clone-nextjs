...
const getDetails = async (req: NextApiRequestWithUser, res: NextApiResponse) => {
  try {
    const { id } = req.query
    await db.connect()
...
const updateDetails = async (req: NextApiRequestWithUser, res: NextApiResponse) => {
  try {
...
const deleteDetails = async (req: NextApiRequestWithUser, res: NextApiResponse) => {
  try {
    if (req.user.role !== roles.ROOT) return sendError(res, 403, 'شما اجازه انجام این عملیات را ندارید')
