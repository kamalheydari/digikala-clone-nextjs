...
import type { IBannerDocument, NextApiRequestWithUser } from 'types'
...
const updateBanner = async (req: NextApiRequestWithUser, res: NextApiResponse) => {
  try {
...
const deleteBanner = async (req: NextApiRequestWithUser, res: NextApiResponse) => {
  try {
    if (req.user.role !== roles.ROOT) return sendError(res, 403, 'شما اجازه انجام این عملیات را ندارید')
