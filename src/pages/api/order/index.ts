...
const createOrder = async (req: NextApiRequestWithUser, res: NextApiResponse) => {
  try {
    const userId = req.user._id
    ...
    cart.forEach((item: ICart) => sold(item.productID, item.quantity, item.inStock, item.sold))
...
const sold = async (_id: ObjectId | string, quantity: number, oldStock: number, oldSold: number): Promise<void> => {
