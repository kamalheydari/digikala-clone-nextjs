...
import type { IReviewDocument, NextApiRequestWithUser } from '@/types'
...
const createReview = async (req: NextApiRequestWithUser, res: NextApiResponse) => {
  try {
...
const deleteReview = async (req: NextApiRequestWithUser, res: NextApiResponse) => {
  try {
    const userId = req.user._id
    ...
    if (review?.user !== userId) return sendError(res, 403, 'شما اجازه انجام این عملیات را ندارید')
...
const updateReview = async (req: NextApiRequestWithUser, res: NextApiResponse) => {
  try {
    if (req.user.role !== roles.ROOT) return sendError(res, 403, 'شما اجازه انجام این عملیات را ندارید')

    await db.connect()

    const review: IReviewDocument | null = await Review.findOneAndUpdate({ _id: req.query.id }, { ...req.body })
    ...
    if (product) {
      const { totalRating, totalReviews } = reviews.reduce(
        (total, item) => {
          if (item.status === 2) {
