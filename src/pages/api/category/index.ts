...
import type { ICategoriesList, ICategoryDocument, NextApiRequestWithUser } from '@/types'
...
const createCategory = async (req: NextApiRequestWithUser, res: NextApiResponse) => {
  try {
...
const getCategories = async (req: NextApiRequestWithUser, res: NextApiResponse) => {
  try {
    await db.connect()
    ...
    const getCategoriesWithChildren = async (): Promise<ICategoriesList[]> => {
      function findChildren(category: ICategoryDocument): ICategoriesList {
        ...
        if (children.length > 0) {
          categoryWithChildren.children = children.map((child: ICategoryDocument) => findChildren(child))
        }

        return categoryWithChildren
      }

      const rootCategories: ICategoryDocument[] = categories.filter((c: ICategoryDocument) => !c.parent)
      const categoriesWithChildren: ICategoriesList[] = rootCategories.map((category: ICategoryDocument) =>
