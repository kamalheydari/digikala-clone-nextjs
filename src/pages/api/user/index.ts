...
import type { IUserDocument, NextApiRequestWithUser } from 'types'
...
const uploadInfo = async (req: NextApiRequestWithUser, res: NextApiResponse) => {
  try {
...
const getUsers = async (req: NextApiRequestWithUser, res: NextApiResponse) => {
  ...
  try {
    if (req.user.role === roles.USER) return sendError(res, 403, 'شما اجازه انجام این عملیات را ندارید')
