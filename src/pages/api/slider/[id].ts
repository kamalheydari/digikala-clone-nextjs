...
import type { ISliderDocument, NextApiRequestWithUser } from 'types'
...
const updateSlider = async (req: NextApiRequestWithUser, res: NextApiResponse) => {
  try {
...
const deleteSlider = async (req: NextApiRequestWithUser, res: NextApiResponse) => {
  try {
    if (req.user.role !== roles.ROOT) return sendError(res, 403, 'شما اجازه انجام این عملیات را ندارید')
